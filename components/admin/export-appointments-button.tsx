...
export function ExportAppointmentsButton() {
  ...
  const exportToExcel = async () => {
    ...
    try {
      ...
      const clinicMap: Record<string, any> = clinics?.reduce((acc, clinic) => ({ ...acc, [clinic.id]: clinic }), {}) || {}
      ...
      const csvData = enrichedAppointments.map((appointment: any) => ({
        "اسم المريض": appointment.profiles?.full_name || "",
        "رقم الهاتف": appointment.profiles?.phone || "",
        "الجنس": getGenderInArabic(appointment.profiles?.gender),
        "العمر": getUserAge(appointment.profiles?.date_of_birth),
